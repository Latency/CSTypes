...
namespace CSTypes {
  public static class Types {
    ...
    public static string CopyFirstWord(string s) {
      return s.TrimStart().Split(' ').First();
    }
    ...
    public static string AdvWord(string s) {
      ...
      return s.Substring(wrd.Length).TrimStart(' ');
    }
    ...
    public static int Dice(int number, int size) {
      return number <= 0 || size <= 0 ? 0 : Number(size, number * size);
    }
    ...
    public static bool IsNumber(string str) {
      return Regex.Match(str.Trim(), @"^-[0-9]+$|^[0-9]+$").Success;
    }
    ...
    public static bool IsValidIP(string addr) {
      return IPAddress.TryParse(addr, out var ip) && (ip.AddressFamily == AddressFamily.InterNetwork || ip.AddressFamily == AddressFamily.InterNetworkV6);
    }
    ...
    public static string NumericSuffix(int number) {
      ...
      if (number == 1 || number > 20 && number % 10 == 1 && number % 100 != 11)
        suf = "st";
      else if (number == 2 || number > 20 && number % 10 == 2 && number % 100 != 12)
        suf = "nd";
      else if (number == 3 || number > 20 && number % 10 == 3 && number % 100 != 13)
        suf = "rd";
      else
        suf = "th";
      return suf;
